	...
	public String addOrder(Model model, Integer shippingId, Integer paymentType, HttpServletRequest req, HttpServletResponse resp,HttpServletResponse response, BigDecimal currentUnitPrice) {
		...
		if (null != buyCartVO) {
			...
			for (CartItemVO item : items) {
				...
				orderItem.setProductId(productId);//��Ʒid
				...
				orderItem.setProductName(productName);//��Ʒ����
				
				String productImage = productTemp.getmain_image();
				orderItem.setProductImage(productImage);//��Ʒ��ͼ
				System.out.println(currentUnitPrice);
				...
				orderItem.setTotalPrice(totalPrice);//��Ʒ�ܼ�
				System.out.println(currentUnitPrice);
				orderItem.setQuantity(item.getAmount());//��Ʒ����
